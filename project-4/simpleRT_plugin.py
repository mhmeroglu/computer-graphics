...
# ----------
        ...
        new_orig = hit_loc + (hit_norm * (0.01)) 
        #
        ...
        diffuse_component = np.array(diffuse_color) * I_light * max(np.dot(light_dir, hit_norm), 0.0)
        ...
        specular_component = np.array(specular_color) * I_light * max(0.0, hit_norm.dot(half_vector)) ** specular_hardness
    ...
    if no_light_hit:
        ambient_component = np.array(diffuse_color) * np.array(ambient_color)
