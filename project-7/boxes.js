...
var shapes = {
   firstWireCube: {points:[], colors:[], start:0, size:0, type: 0},
   secondWireCube: {points:[], colors:[], start:0, size:0, type: 0},
...
{
   shapes.firstWireCube.points.push(cubeVerts[wireCubeLookups[i]]);
   shapes.firstWireCube.colors.push(white);
}
for (var i =0; i < wireCubeLookups.length; i++)
{
   shapes.secondWireCube.points.push(cubeVerts[wireCubeLookups[i]]);
   shapes.secondWireCube.colors.push(pink);
}
...
window.onload = function init() {
  ...
  loadShape(shapes.firstWireCube, gl.LINE_STRIP);
  loadShape(shapes.secondWireCube, gl.LINE_STRIP);
  ...
  p = ortho(-3 * aspect, 3 * aspect, -3, 3, -100, 100);
...
function render() {
	...
	mv = lookAt(eye,at,up);
   //mv = translate(0, 0, -15);
   //mv = mult(mv, rotate(270, [1, 0, 0]));
   mv = mult(mv, rotate(25, [1, 0, 0]));
   mv = mult(mv, rotate(25, [0, 1, 0]));
   ...
   gl.uniformMatrix4fv(mvLoc, gl.TRUE, flatten(transpose(mv)));
   gl.drawArrays(shapes.firstWireCube.type, shapes.firstWireCube.start, shapes.firstWireCube.size);
   
   //Change solid to wire
   mv = mult(mv, rotate(45, [0, 1, 0]));
   mv = mult(mv, translate(0,1,0));
   gl.uniformMatrix4fv(mvLoc, gl.TRUE, flatten(transpose(mv)));
   gl.drawArrays(shapes.secondWireCube.type, shapes.secondWireCube.start, shapes.secondWireCube.size);
}
