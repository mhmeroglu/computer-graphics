...
//Adding model state variables
var armx = 0, army = 0, fingerx = 0, fingery = 0;
...
window.onload = function init() {
   ...
   o = ortho(-3.4*aspect, 3.4*aspect, -3.4, 3.4, 1.0, 20.0);
...
function animate()
{
    ...
    solidCube = shapes.solidCube;
    wireCube = shapes.wireCube;
    shapeObject = solidCube;
...
function render() {
   ...
   var armShape = shapeObject;
	...
	matStack.push(mv);
      ...
      // Arm Joint
      mv = mult(mv, rotate(armx, vec3(1, 0, 0)));
      mv = mult(mv, rotate(army, vec3(0, 1, 0)));
      ...
      // Position Finger-1
      mv = mult(mv, rotate(fingerx, vec3(1, 0, 0)));
      // Position Finger-1 Joint
      mv = mult(mv, translate(1.2, 0, 0.0));
      // Scale and Draw Finger-1
      matStack.push(mv)
         mv = mult(mv, translate(0, -0.4, 0));
      ...
      for (var i = 0; i < 3; i++) {
      matStack.push(mv);
         mv = mult(mv, translate(0, 0.4, (i - 1) * 0.4)); 
         mv = mult(mv, rotate(-45, vec3(0, 0, 1))); 
         mv = mult(mv, scale(0.5, 0.2, 0.2)); 
         gl.uniformMatrix4fv(mvLoc, gl.FALSE, flatten(transpose(mv)));
         gl.drawArrays(armShape.type, armShape.start, armShape.size);
      ...
      for (var i = 0; i < 3; i++) {
         matStack.push(mv);
            mv = mult(mv, translate(0, 0.6, (i - 1) * 0.4));
            mv = mult(mv, scale(0.5, 0.2, 0.2)); 
            gl.uniformMatrix4fv(mvLoc, gl.FALSE, flatten(transpose(mv)));
            gl.drawArrays(armShape.type, armShape.start, armShape.size);
         //Undo Scale
...
function handleKeys(timePassed) 
{
   ...
   //Fingers Updates
   if (shift && isPressed("A") || !shift && isPressed("A")) {
      if (fingerx > -90) fingerx = (fingerx - d);
      else fingerx = -90;
   }
   if (shift && isPressed("B") || !shift && isPressed("B")) {
      if (fingerx < 90) fingerx = (fingerx + d);
      else fingerx = 90;
   }
   if (shift && isPressed("M") || !shift && isPressed("M")) {
      if (fingery > -20) fingery = (fingery - d);
      else fingery = -20;
   }
   if (shift && isPressed("N") || !shift && isPressed("N")) {
      if (fingery < 20) fingery = (fingery + d);
      else fingery = 20;
   }
   ...
   // Shape Updates
   if (isPressed("T")) {
      if (shapeObject == solidCube) {
         shapeObject = wireCube;
      }
      else {
         shapeObject = solidCube;
      }
